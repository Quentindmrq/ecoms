...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("select jhiOrder from Order jhiOrder where jhiOrder.owner.login = ?#{principal.username}  and jhiOrder.purchased = true")
    Page<Order> findByOwnerIsCurrentUser(Pageable pagerequested);

    @Query("select jhiOrder from Order jhiOrder where jhiOrder.owner.login = ?#{principal.username} and jhiOrder.purchased = false")
